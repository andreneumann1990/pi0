...
# Plots the histogram of SS, BSS and BSS_shrinkage over L tests, respectively,
# and calculates the resulting mean bias and root mean squared error.
main <- function() {
  seed <- seed(78958) #78958
  ...
  pi0_BSS_shrink <- c(bias = mean(results[, "BSS_shrinkage"] - default$pi0),
               RMSE = sqrt(mean((results[, "BSS_shrinkage"] - default$pi0)^2)))
  plot_histograms(results, default$pi0)

  results <- list(pi0_hat = results, pi0 = default$pi0, pi0_hat_SS = pi0_SS,
                  pi0_hat_BSS = pi0_BSS, pi0_hat_BSS_shrinkage = pi0_BSS_shrink)
  save(results, file = "2018-11-19-pi0-histogram-2.RData")
  results
}
...
estimate_pi0 <- function(rho_, n_, m_, pi0_, mu_, lambda_, L, B) {
  ...
  foreach(l = 1:L, .export = lsf.str(.GlobalEnv),
    ...
    c <- qt(1 - lambda_/2, kn_ - 1)
    ncp <- sqrt(kn_)*mu_hat/sd_hat
    distr_p_star <- pt(-c, kn_ - 1, ncp) + pt(c, kn_ - 1, ncp, lower.tail = FALSE)
    BSS_shrinkage <- (1 - mean(distr_p_star))/(1 - lambda_)

    p <- p_values(sqrt(n_) * mu_hat/sd_hat, n_)
    c(SS = calculate_Pi0(p, lambda_), BSS = BSS, BSS_shrinkage = BSS_shrinkage)
  }
}
...
plot_histograms <- function(results, pi0) {
  ...
  ggsave("2018-11-19-pi0-histogram-2-SS.png", width = 6.4, height = 6.4)
  print(gg)
  ...
  ggsave("2018-11-19-pi0-histogram-2-BSS.png", width = 6.4, height = 6.4)
  print(gg)
  ...
  gg <- ggplot() +
    ...
    labs(x = expression(hat(pi)[0] - pi[0]), colour = "") +
    coord_cartesian(xlim = c(-0.5, 0.5)) + # zoom
    geom_histogram(aes(results[, "BSS_shrinkage"] - pi0, stat(density)), binwidth = 0.01)
  ggsave("2018-11-19-pi0-histogram-2-BSS_shrinkage.png", width = 6.4, height = 6.4)
  print(gg)
}
