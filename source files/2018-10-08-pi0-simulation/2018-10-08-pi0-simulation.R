...
# This simulation compares the mean bias and root mean squared error of the
# Schweder-Spjotvoll (SS) estimator and the bootstrap SS (BSS) estimator in
# different settings.
main <- function() {
  seed <- seed(355073) #355073
...
compare_biasAndRMSE <- function(rho, n, m, pi0, mu, lambda, default, L, B, seed) {
  ...
  results$rho[1:l, 1] <- "SS"
  results$rho[1:l, 2] <- rho
  results$rho[(l + 1):(2*l), 1] <- "BSS"
  ...
  results$pi0[1:l, 1] <- "SS"
  results$pi0[1:l, 2] <- pi0
  results$pi0[(l + 1):(2*l), 1] <- "BSS"
  ...
  results$lambda[1:l, 1] <- "SS"
  results$lambda[1:l, 2] <- lambda
  results$lambda[(l + 1):(2*l), 1] <- "BSS"
